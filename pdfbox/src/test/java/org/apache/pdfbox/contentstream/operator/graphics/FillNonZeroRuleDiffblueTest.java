...
class FillNonZeroRuleDiffblueTest {
  /**
   * Test {@link FillNonZeroRule#process(Operator, List)}.
   * <ul>
   ...
   * </ul>
   * <p>
   * Method under test: {@link FillNonZeroRule#process(Operator, List)}
   */
  @Test
  ...
  @MethodsUnderTest({"void FillNonZeroRule.process(Operator, List)"})
  void testProcess_givenPageDrawerFillPathDoesNothing_whenArrayList_thenCallsFillPath() throws IOException {
    ...
    FillNonZeroRule fillNonZeroRule = new FillNonZeroRule(context);
    Operator operator = Operator.getOperator("Operator");

    // Act
    fillNonZeroRule.process(operator, new ArrayList<>());

    // Assert
    verify(context).fillPath(eq(1));
  }
}
